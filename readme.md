...
2. ```mkdir -p ./dags ./logs ./plugins``` to setup airflow directories
3. ```echo -e "AIRFLOW_UID=$(id -u)" > .env``` to set up the permissions of curent user to virtual environment.
...
1. ```docker-compose up airflow-init``` to initialize airflow database
2. ```docker-compose up``` tostart running airflow. you could access airflow UI at this point using localhost:8080 with username == airflow and password == airflow
...
2. Insert the following commands, in this case we need to install matplotlib.
    ...
    ```
4. use the command as follows to build up to build up docker image, in this case we named it as my-de01-airflow.
    ```docker build -t my-de01-airflow .```
5. go to .yaml file and check the x-airflow-common > image section to make sure if airflow are built up with on our customised image.
